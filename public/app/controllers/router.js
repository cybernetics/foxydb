steal(
	...
	'app/controllers/insight.js',
	'lib/jquery-cookie/jquery.cookie.js',
	'semantic/javascript/semantic.js'
).then(
	'app/views/layout/header.ejs',
	'app/views/layout/content.ejs',
	'app/views/layout/layout.ejs',
	'app/views/layout/controller.ejs',
	'app/views/layout/insight.ejs',
	function() {
		can.Control(
			'Controller.Router',
			{
			...
			},
			{
				...
				init: function(element, options) {
					...
					this.element.html('app/views/layout/layout.ejs', {});
				...
				loadPage: function(controller, action, options) {
					...
					if(['Dashboard','Insight'].indexOf(controller) == -1) {
						...
						if(this.currentController != controller) {
							this.element.find('.controller').replaceWith('app/views/layout/controller.ejs', {});
						...
						if(controller == 'Dashboard') {
							...
							if(this.element.find('.insight#insight_' + id).length == 0) {
								var el = $('.pagecontent:last').after('app/views/layout/insight.ejs', {id: id});
